...
export function calculateProjection(inputs) {
  const {
    ...
    carFundGoal = 0,
    // Events: array of objects { type: 'living'|'remittance', month: 'YYYY-MM', value: number }
    events = [],
    // v1.6: optimize loan payment automatically by allocating all leftover cash to the student loan
  ...
  let monthCounter = 0;
  // Maintain override values for living and remittance. These values can be
  // modified by timeline events and persist until changed again. Start with
  // the provided living and remittance inputs.
  let livingOverride = living;
  let remittanceOverride = remittance;
  // Overrides for contributions that can be modified via events. These mirror
  // the user inputs and persist until changed by a matching event. See the
  // events processing below.
  let additionalSavingsOverride = additionalSavings;
  let carFundMonthlyOverride = carFundMonthly;
  let loanExtraOverride = loanExtra;
  ...
  while (currentDate <= payoffDate && monthCounter < 240) { // safety limit of 20 years
    ...
    // Check each event to see if it occurs this month. If so, update the
    // corresponding override (living or remittance) to the new value. Events
    // persist for all subsequent months until superseded by another event.
    if (events && Array.isArray(events)) {
      for (const ev of events) {
        if (ev && ev.month === monthLabel) {
          if (ev.type === 'living') {
            livingOverride = ev.value;
          } else if (ev.type === 'remittance') {
            remittanceOverride = ev.value;
          } else if (ev.type === 'additionalSavings') {
            additionalSavingsOverride = ev.value;
          } else if (ev.type === 'carFundMonthly') {
            carFundMonthlyOverride = ev.value;
          } else if (ev.type === 'loanExtra') {
            loanExtraOverride = ev.value;
          }
        }
      }
    }
    ...
    // Use the override values that may have been updated by events.
    const livingCurrent = livingOverride + carOngoingApplicable;
    ...
    // subtracted later.
    const availableForDebt = netMonthly - livingCurrent - remittanceOverride;
    ...
    let extraLoanPayment = needsEmergencyBuffer ? 0 : loanExtraOverride;
    ...
    // NOTE: loanEnding will be computed after any optimization step below
    let loanEnding;
    let carLoanEnding = carLoanBalance + carLoanInterest - carLoanPayment;
    ...
    // --- Optimize loan payment ---
    // If the user opts to allocate all leftover cash to the student loan, apply
    // the leftover (if positive) as an additional payment to the loan. Limit
    // the extra payment to the remaining principal plus interest for this month.
    if (optimizeLoanPayment && loanBalance > 0 && leftoverAfterDebt > 0) {
      const maxExtra = Math.max(0, loanBalance + loanInterest - loanPayment);
      const extraPayment = Math.min(leftoverAfterDebt, maxExtra);
      loanPayment += extraPayment;
      leftoverAfterDebt -= extraPayment;
    }

    // After any optimization, compute the new ending loan balance
    ...
    let carContributionPlanned = carFundMonthlyOverride;
    ...
    let additionalSavingsActual = additionalSavingsOverride;
    // depositCash will accumulate all cash deposited this month
    let depositCash = 0;

    // ----------------------------------------------------------------------
    // Deposit logic with minimum cash buffer enforcement
    //
    // We always ensure that cash savings meets or exceeds `minCashBuffer`.
    // At the start of each month we may have positive leftover after paying
    // debts (`leftoverAfterDebt`). We also have optional contributions:
    // `carContributionActual` and `additionalSavingsActual`. If our cash
    // savings balance is below `minCashBuffer`, we redirect leftover and
    // contributions to cash savings until either the minimum is reached or
    // we run out of funds. After meeting the minimum (or if we already
    // exceed it), any positive cash left after contributions is deposited
    // to cash savings. Negative values simply result in negative
    // `cashRemaining` later, indicating a shortfall.
    //
    // We do not use `needsEmergencyBuffer` here, since the emergency case
    // (shortfall) and the minimum buffer case are the same threshold.
    // ----------------------------------------------------------------------
    {
      // Convert leftoverAfterDebt into a mutable leftover pool. Only the
      // positive portion of leftover can be deposited.
      let leftover = Math.max(0, leftoverAfterDebt);
      // Determine how much we need to deposit to reach the minimum buffer
      const shortfall = minCashBuffer > 0 ? Math.max(0, minCashBuffer - cashSavingsBalance) : 0;
      if (shortfall > 0) {
        let depositRemaining = shortfall;
        // Deposit from leftover first
        const fromLeftover = Math.min(leftover, depositRemaining);
        depositCash += fromLeftover;
        leftover -= fromLeftover;
        depositRemaining -= fromLeftover;
        // Deposit from additional savings
        if (depositRemaining > 0 && additionalSavingsActual > 0) {
          const fromAddl = Math.min(additionalSavingsActual, depositRemaining);
          depositCash += fromAddl;
          additionalSavingsActual -= fromAddl;
          depositRemaining -= fromAddl;
        }
        // Deposit from car fund contribution
        if (depositRemaining > 0 && carContributionActual > 0) {
          const fromCar = Math.min(carContributionActual, depositRemaining);
          depositCash += fromCar;
          carContributionActual -= fromCar;
          depositRemaining -= fromCar;
        }
        // If depositRemaining is still positive after exhausting all sources,
        // we cannot fully meet the minimum this month. The remaining
        // shortfall will be reflected in a negative cashRemaining.
      }
      // Compute cash available after contributions. This equals the leftover
      // pool minus the updated car contribution plus the updated additional
      // savings. If this is positive, deposit it to cash savings. If
      // negative, it becomes cashRemaining later.
      const afterContrib = leftover - carContributionActual + additionalSavingsActual;
      if (afterContrib > 0) {
        depositCash += afterContrib;
        // Reduce leftover accordingly. Although we don’t use leftover
        // directly afterwards, reducing it helps document the flow.
        leftover -= afterContrib;
      }
      // At this point, depositCash contains the total amount moved into
      // cash savings this month and carContributionActual/additionalSavingsActual
      // reflect the contributions not diverted to the cash buffer.
    }
    ...
    // After deposit is determined, compute the cash remaining. In cases where
    // either an emergency buffer is being built or the minimum buffer is
    // enforced, we deposit all available cash to savings and treat cash
    // remaining as zero. Otherwise, cashRemaining reflects any shortfall
    // (negative) or extra cash left after making contributions and deposits.
    let cashRemaining;
    if (needsEmergencyBuffer || (minCashBuffer > 0 && cashSavingsBalance < minCashBuffer)) {
      cashRemaining = 0;
    } else {
